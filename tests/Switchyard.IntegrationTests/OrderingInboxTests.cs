...
public sealed class OrderingInboxTests
{
    ...
    public async Task ProcessesMessageOnceAndReplaysWithoutRepeatingOutboxSideEffect()
    {
        ...
        var processor = new EfOrderingInboxMessageProcessor(
            dbContext,
            new FixedTimeProvider(now));
        var outbox = new EfOrderingOutboxStore(dbContext);
    ...
    public async Task HandlerFailureRollsBackInboxAndOutboxBeforeRetry()
    {
        ...
        await using (var failedContext = CreateDbContext(postgres.GetConnectionString()))
        {
            var processor = new EfOrderingInboxMessageProcessor(
                failedContext,
                new FixedTimeProvider(now));
            var outbox = new EfOrderingOutboxStore(failedContext);
        ...
        var retryProcessor = new EfOrderingInboxMessageProcessor(
            retryContext,
            new FixedTimeProvider(now.AddSeconds(1)));
        var retryOutbox = new EfOrderingOutboxStore(retryContext);
    ...
    public async Task ConcurrentDuplicateDeliveriesInvokeHandlerOnce()
    {
        ...
        var firstProcessor = new EfOrderingInboxMessageProcessor(
            firstContext,
            new FixedTimeProvider(now));
        var secondProcessor = new EfOrderingInboxMessageProcessor(
            secondContext,
            new FixedTimeProvider(now));
        ...
        CancellationToken cancellationToken)
    {
        var postgres = new PostgreSqlBuilder("postgres:18-alpine")
            .WithDatabase("switchyard_ordering_inbox_test")
